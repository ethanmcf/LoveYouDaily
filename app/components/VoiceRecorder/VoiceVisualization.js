...
import { View, StyleSheet, Animated, Dimensions, FlatList, Text } from 'react-native';
...
function VoiceVisualization({}) {
    ...
    const animateBar = (index, amplification) => {
        ...
        for(i=0;i<numIncrements;i++){
            ...
            toValue = i <= median ? toValue + incrementValue : toValue - incrementValue 
            delay = i <= median ? delay - randomDelayChange : delay + randomDelayChange
            ...
            Animated.sequence([
                ...
                index = Math.floor(Math.random() * (animationList.length - 1 - numIncrements))
        ...
        <View style={styles.container}>
            <FlatList 
                ...
                renderItem={({index}) => {
                    return( 
                        <Animated.View 
                            style={[styles.bar, {height:15, transform: [{scaleY: animationList[index]}]}]} 
                            key={index}/>
                    )
                }}
